Lets make the "jobs" database table based on the screenshots attached. (make sure to create the table with compliant name). I have attached multiple screenshots showing different combinations of the columns to get the full picture of all the columns
The column names should be Pascal notation so that "Scheduled Date" becomes "StatusScheduledDate" etc.
...
I want the UI to create/edit the rows to be much like the Nextcloud Tables. So an editble table in the "Jobs" tab.

Except not all columns are in the visual table. Only [Title][Mode],[Scheduled Date] The rest of the columns are only visible in the create/edit dialogbox

There should be a menu {...} (no visual brackets) on the top right of the visual table on top of the columnnames if not enouth space to display cucurrently only 2 menu item namely "Create new","Import". lets define the actions for "Import" later. But the "Create new" menu item should create a new row. The creation or edit of a row should be in a dialogbox just like Nextcloud tables app. (later we will defines conditional validation and visibity for the columns in the create/edit dialogbox)
